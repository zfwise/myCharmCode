...
:Authors:    Fan Zhang
:Date:       3/2013
:Note:  This exactly follows the paper. I have a improved version, which
faster and a little bit different from what the paper described. But the
security proof still stand.
'''
...
class IBE_Chen12(IBEnc):
    ...
    def extract(self, msk, ID):
        _ID = group.hash(ID)
        r = group.random(ZR)
        sk_id1 = (msk['gD11']**(msk['alpha']+r*_ID))/(msk['gD21']**r)
        sk_id2 = (msk['gD12']**(msk['alpha']+r*_ID))/(msk['gD22']**r)
        sk_id3 = (msk['gD13']**(msk['alpha']+r*_ID))/(msk['gD23']**r)
        sk_id4 = (msk['gD14']**(msk['alpha']+r*_ID))/(msk['gD24']**r)
